...
def main():
    ...
    print(f"{'Category':<25s} {'W':>4s} {'L':>4s} {'P':>4s} {'Total':>6s} {'Win%':>6s}")
    print("-" * 55)
    print(f"{'⭐ Top Performer':<25s} {tagged_w:>4d} {tagged_l:>4d} {tagged_p:>4d} {tagged_resolved:>6d} {tagged_pct:>5.1f}%")
    print(f"{'   (combo match)':<25s} {sum(1 for r in tagged if r['_tp_reason'] == 'combo' and r['result'] == 'won'):>4d} {sum(1 for r in tagged if r['_tp_reason'] == 'combo' and r['result'] == 'lost'):>4d} {sum(1 for r in tagged if r['_tp_reason'] == 'combo' and r['result'] == 'push'):>4d}")
    print(f"{'   (hour match)':<25s} {sum(1 for r in tagged if r['_tp_reason'] == 'hour' and r['result'] == 'won'):>4d} {sum(1 for r in tagged if r['_tp_reason'] == 'hour' and r['result'] == 'lost'):>4d} {sum(1 for r in tagged if r['_tp_reason'] == 'hour' and r['result'] == 'push'):>4d}")
    print(f"{'No badge':<25s} {untagged_w:>4d} {untagged_l:>4d} {untagged_p:>4d} {untagged_resolved:>6d} {untagged_pct:>5.1f}%")
    print()
    ...
    for r in tagged:
        ...
        if key not in combo_stats:
            combo_stats[key] = {"won": 0, "lost": 0, "push": 0}
    ...
    for r in tagged:
        ...
        if key not in hour_stats:
            hour_stats[key] = {"won": 0, "lost": 0, "push": 0}
    ...
    for r in tagged[-15:]:
        ...
        reason = r["_tp_reason"]
        print(
            f"  [{tag}] {date_str:16s} | {r['signal_type']:25s} | "
