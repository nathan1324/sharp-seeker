...
class TestTimingStats:
    @pytest.mark.asyncio
    async def test_timing_stats_groups_live_and_pregame(self, repo):
        """get_performance_stats_by_timing groups results correctly."""
        # Insert a live signal (is_live=True) and a pregame signal (is_live=False)
        await repo.record_signal_result(
            event_id="g1", signal_type="steam_move", market_key="h2h",
            outcome_name="Team A", signal_direction="up", signal_strength=0.9,
            signal_at="2025-01-15T20:00:00", is_live=True,
        )
        await repo.record_signal_result(
            event_id="g2", signal_type="steam_move", market_key="h2h",
            outcome_name="Team B", signal_direction="up", signal_strength=0.8,
            signal_at="2025-01-15T18:00:00", is_live=False,
        )
        # Resolve both
        await repo.resolve_signal("g1", "steam_move", "h2h", "Team A",
                                  "2025-01-15T20:00:00", "won")
        await repo.resolve_signal("g2", "steam_move", "h2h", "Team B",
                                  "2025-01-15T18:00:00", "lost")

        stats = await repo.get_performance_stats_by_timing()
        assert "live" in stats
        assert "pregame" in stats
        assert stats["live"]["won"] == 1
        assert stats["live"]["total"] == 1
        assert stats["pregame"]["lost"] == 1
        assert stats["pregame"]["total"] == 1

    @pytest.mark.asyncio
    async def test_timing_stats_excludes_null_is_live(self, repo):
        """Signals with is_live=NULL should not appear in timing stats."""
        await repo.record_signal_result(
            event_id="g3", signal_type="steam_move", market_key="h2h",
            outcome_name="Team C", signal_direction="up", signal_strength=0.7,
            signal_at="2025-01-15T19:00:00",
        )
        await repo.resolve_signal("g3", "steam_move", "h2h", "Team C",
                                  "2025-01-15T19:00:00", "won")
