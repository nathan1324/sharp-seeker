...
| `BOOKMAKERS` | `["draftkings","fanduel","betmgm","pinnacle","williamhill_us","betrivers","fanatics","hardrockbet"]` | Bookmakers to monitor (JSON array) |
...
| `PINNACLE_SPREAD_THRESHOLD` | `1.0` | Spread divergence threshold (points) |
| `PINNACLE_TOTALS_THRESHOLD` | `1.0` | Totals divergence threshold (points) |
...
| `X_FREE_PLAY_EXCLUDED_SPORTS` | `[]` | Sport keys benched out of free plays even if they match a combo (JSON array). Use to park a whole sport whose edge is still under test without editing the combo list |
| `X_FREE_PLAY_EXCLUDED_COMBOS` | `[]` | `type:sport:market` combos carved out of free plays even if they match a whitelist combo (JSON array, same `*` wildcard syntax). Use to drop one market for one sport while a blanket combo (e.g. `pinnacle_divergence:*:totals`) still serves the others |
